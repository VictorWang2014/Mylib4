...
- (void)initialView {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(vcHeaderView)]) {
        _headerView = [self.delegate vcHeaderView];
...
//        _headerView.backgroundColor = [UIColor randomColor];
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(vcSegmentView)]) {
        _segmentView = [self.delegate vcSegmentView];
...
- (void)scrollToPageIndex:(int)index animation:(BOOL)animate {
    [_collectionView scrollToItemAtIndexPath:[NSIndexPath indexPathForRow:index inSection:0] atScrollPosition:UICollectionViewScrollPositionNone animated:animate];
}
